...
class Deserialiser {
  ...
  process_related_item(relitem, chain) {
    ...
    if (this.included && inc_key in this.included) {
      ...
      if ('relationships' in relitem_inc) {
        this.process_relationships(relitem_inc.relationships, chain).then(resolved_relations => {
    ...
    return new Promise((resolve, reject) => {
      ...
      let res = rel_types.reduce((acc, rel_type) => {
        ...
        if (_isPlainObject(reldat)) {
          ...
          acc[rel_type] = reldat.filter(r => !(self.generate_key(r) in chain)).map(r => {
  ...
  process_item(item) {
    ...
    if ('relationships' in item) {
      this.process_relationships(item.relationships).then(rels => {
  ...
  deserialise(jsonapi) {
    ...
    return new Promise((resolve, reject) => {
      ...
      self.process_included(jsonapi).then(() => {
        if (_isArray(jsonapi.data)) {
          resolve(Promise.all(jsonapi.data.map(item => self.get_object_attributes(item))));
